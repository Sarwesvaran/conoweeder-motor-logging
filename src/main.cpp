...
char ssid[] = "DataCorp-Intranet-IoT";
char pass[] = "Sm@rtDevices@1025";
...
bool syncTimeWithRetries()
{
  ...
  for (int attempt = 1; attempt <= TIME_SYNC_RETRIES; attempt++)
  {
    ...
    if (now > 1600000000)
    { // If we got a valid time (after year 2000)
...
void sendResult(String time, float volt, float current, float power, int rpm, int totalRuntimeMinutes)
{
  ...
  for (int attempt = 1; attempt <= MAX_SHEET_RETRIES; attempt++)
  {
    ...
    valueRange.set("values/[0]/[0]", "Motor 2");
...
void setup()
{
  ...
  EEPROM.begin(512);
  // EEPROM.put(eepromRuntimeAddr, 0);
  // EEPROM.commit();
  delay(1000);
